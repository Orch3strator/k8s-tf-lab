...
if [[ ! -f "${STATUS_FILE}" ]]; then
    SCRIPT_DATA_FILE_DNS="${DIR_NAME}/config/eth0.txt"

    # Script for nsupdate
    ADDR=$(nmcli device show eth0 | grep IP4.ADD | awk '{print $2}' | awk -F/ '{print $1}')
    HOST=$(hostname -s)
    DNSSERVER=$(nmcli device show eth0 | grep IP4.DNS | awk '{print $2}' | head -1)
    # DNSZONE=$(nmcli device show eth0 | grep IP4.DOMAIN | awk '{print $2}' | head -1)
    DNSZONE=$(hostname -d)

    echo "server ${DNSSERVER}" | sudo tee "${SCRIPT_DATA_FILE_DNS}"
    echo "zone ${DNSZONE}" | sudo tee -a "${SCRIPT_DATA_FILE_DNS}"
    echo "update delete ${HOST}.${DNSZONE} A" | sudo tee -a "${SCRIPT_DATA_FILE_DNS}"
    echo "update add ${HOST}.${DNSZONE} 600 A $ADDR" | sudo tee -a "${SCRIPT_DATA_FILE_DNS}"
    echo "show" | sudo tee -a "${SCRIPT_DATA_FILE_DNS}"
    echo "send" | sudo tee -a "${SCRIPT_DATA_FILE_DNS}"
    ...
    if [[ -f "${SCRIPT_DATA_FILE}" ]]; then
        ...
        while IFS= read -r obj; do
            # echo "JSON Key: ${obj}"
            VM_DNS_NAME=$(echo "${obj}" | jq -r '.name')
            VM_DNS_DOMAIN=$(echo "${obj}" | jq -r '.domain')
            VM_IP=$(echo "${obj}" | jq -r '.ip')

            echo " ---- "
            echo -e " ${Cyan}Host Name  : ${Color_Off}${VM_DNS_NAME}"
            echo -e " ${Cyan}Domain     : ${Color_Off}${VM_DNS_DOMAIN}"
            echo -e " ${Cyan}IP Address : ${Color_Off}${VM_IP}"

            # keep track of deployment status
            STATUS_STEP="dns"
            STATUS_FILE="${STATUS_FILE_PREFIX}.${STATUS_STEP}.${VM_DNS_NAME}.txt"

            if [[ ! -f "${STATUS_FILE}" ]]; then
                echo "${VM_IP}    ${VM_DNS_NAME}.${VM_DNS_DOMAIN}    ${VM_DNS_NAME}" | sudo tee -a /etc/hosts
                sudo touch "${STATUS_FILE}"
            fi
        done < <(echo "${SCRIPT_DATA}" | jq -r '.VM.LINUX' | jq -c '.[]')
