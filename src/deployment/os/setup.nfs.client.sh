...
SCRIPT_PURPOSE="NFS Client Management"
...
echo " -----------------------------------------------"
SCRIPT_ACTION="NFS Client Config"
...
if [[ ! -f "${STATUS_FILE}" ]]; then
    ...
    if [[ -f "${SCRIPT_DATA_FILE}" ]]; then
        ...
        while IFS= read -r obj; do
            ...
            NFS_TARGET_DIR=$(echo "${obj}" | jq -r '.target')
            ...
            echo -e " ${Cyan}Target Folder   : ${Color_Off}${NFS_TARGET_DIR}"

            if [ ! -d "${NFS_TARGET_DIR}" ]; then
                sudo mkdir -p "${NFS_TARGET_DIR}"
            fi
            ...
            if [[ ! -f "${STATUS_FILE}" ]]; then
                sudo mount -t nfs4 "${NFS_SERVER}":"${NFS_SOURCE_DIR}" "${NFS_TARGET_DIR}"
                echo "${NFS_SERVER}:${NFS_SOURCE_DIR}    ${NFS_TARGET_DIR}    nfs4    defaults 0 0" | sudo tee -a /etc/fstab
                sudo touch "${STATUS_FILE}"
            fi

            # Test NFS connection
            # keep track of deployment status
            STATUS_STEP="connection.test"
            NFS_TEST_FILE="${NFS_TARGET_DIR}/.${STATUS_STEP}.${HOST_FQDN}.txt"
            sudo touch "${NFS_TEST_FILE}"
            SCRIPT_ACTION=" NFS Mount Tested: ${DATE_TODAY}"
            echo "${SCRIPT_ACTION}" | sudo tee -a "${NFS_TEST_FILE}"
            sudo sh -c "echo ' SCRIPT_ACTION : ${SCRIPT_ACTION}' >> '${LOG_FILE}'"

        done \
            < <(echo "${SCRIPT_DATA}" | jq -r '.NFS.DIR' | jq -c '.[]')
