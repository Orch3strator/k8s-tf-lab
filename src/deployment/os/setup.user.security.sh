...
# file name: setup.user.security.sh
################################################################################
# License                                                                      #
################################################################################
function license() {
...
SCRIPT_PURPOSE="authentication and security"
...
echo " -----------------------------------------------"
SCRIPT_ACTION="fix security & pam issues for ctm"
...
if [[ ! -f "${STATUS_FILE}" ]]; then
    ...
    # pamd.d configuration to support CTM MFT SFTP login
    # keep track of deployment status
    STATUS_STEP="pam"
    STATUS_FILE="${STATUS_FILE_PREFIX}.${STATUS_STEP}.${USER_GROUP_PRIMARY}.txt"
    if [[ ! -f "STATUS_FILE" ]]; then
        echo -e "auth       include      system-auth" | sudo tee -a /etc/pam.d/passwd
        echo -e "account    include      system-auth" | sudo tee -a /etc/pam.d/passwd
